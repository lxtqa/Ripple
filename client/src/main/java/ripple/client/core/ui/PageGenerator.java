...
public final class PageGenerator {
    ...
    private static void appendSideBar(StringBuilder stringBuilder, String currentFunction) {
        ...
        stringBuilder.append("                <ul class=\"nav flex-column mb-2\">\n");
        PageGenerator.buildSidebarItem(stringBuilder, "已订阅配置", "/Subscription/Get", "share-2", currentFunction);
        PageGenerator.buildSidebarItem(stringBuilder, "订阅新配置", "/Subscription/Add", "link", currentFunction);
        PageGenerator.buildSidebarItem(stringBuilder, "取消订阅", "/Subscription/Remove", "delete", currentFunction);
        stringBuilder.append("                </ul>\n");
        PageGenerator.buildSidebarTitle(stringBuilder, "基本信息");
        stringBuilder.append("                <ul class=\"nav flex-column mb-2\">\n");
        PageGenerator.buildSidebarItem(stringBuilder, "已连接的服务器", "/Info/Server", "server", currentFunction);
        stringBuilder.append("                </ul>\n");
    ...
    public static String buildPage(String pageTitle, String currentFunction, String content) {
        ...
        PageGenerator.appendNavigationBar(stringBuilder, "Ripple Client");
