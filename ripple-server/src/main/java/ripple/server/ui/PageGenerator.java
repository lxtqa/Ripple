...
public final class PageGenerator {
    ...
    private static void appendSideBar(StringBuilder stringBuilder, String currentFunction, StringTable stringTable) {
        ...
        stringBuilder.append("                <ul class=\"nav flex-column mb-2\">\n");
        PageHelper.buildSidebarItem(stringBuilder, stringTable.serverGetSubscription(), Endpoint.UI_GET_SUBSCRIPTION, "share-2", currentFunction);
        stringBuilder.append("                </ul>\n");
        PageHelper.buildSidebarTitle(stringBuilder, stringTable.clusterManagement());
        stringBuilder.append("                <ul class=\"nav flex-column mb-2\">\n");
        PageHelper.buildSidebarItem(stringBuilder, stringTable.serverCluster(), Endpoint.UI_SERVER_CLUSTER, "server", currentFunction);
        PageHelper.buildSidebarItem(stringBuilder, stringTable.connectedClients(), Endpoint.UI_CLIENT_CLUSTER, "list", currentFunction);
        stringBuilder.append("                </ul>\n");
